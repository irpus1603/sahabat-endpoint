...
class ChatCompletionRequest(BaseModel):
    ...
    model: str = Field(
        ...
        description=(
            ...
            "The value must match the MODEL_NAME configured in the server's .env file. "
            "Available models:\n"
            "- `Qwen/Qwen3-30B-A3B-GGUF` — Qwen3 30B (GGUF backend, Q4_K_M quant, recommended)\n"
            "- `Sahabat-AI/gemma2-9b-cpt-sahabatai-v1-instruct` — Sahabat-AI Gemma2 9B (transformers backend)\n"
            "Note: the server loads only one model at a time; sending a different model name "
            "than the one currently loaded does not switch models — it is passed through for "
            "logging/response labelling only."
        )
    )
