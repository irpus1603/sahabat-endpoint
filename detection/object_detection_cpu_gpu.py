...
def get_model():
    ...
    if det_model is None:
        ...
        det_model = YOLO("models/yolo11n.pt", task='detect')
        det_model.to(device)
...
def calculate_frame_difference(frame1, frame2):
    """Calculate difference between two frames using structural similarity"""
    if frame1 is None or frame2 is None:
        return float('inf')  # Force processing if no previous frame
...
def process_video_stream(frame_queue, camera): 
    ...
    skip_frames = 2  # Process every 2nd frame to reduce load but maintain smoothness

    # Frame comparison variables
    previous_frame = None
    frame_diff_threshold = 8000  # Adjust based on your needs - higher = less sensitive

    try:
        while True:
                try:
                    frame = frame_queue.get(timeout=1)  # 1 second timeout for smoother streaming
                except Empty:
                    logger.warning("No frames received in 1 second, continuing...")
                    continue
                ...
                # Skip frames to reduce processing load but still stream them
                
                process_frame = frame_skip_counter < skip_frames
                if process_frame:
                    frame_skip_counter = 0
                ...
                # Only run inference on processed frames to reduce load
                results = None
                
                if process_frame:
                    # Check frame difference before running YOLO inference
                    frame_diff = calculate_frame_difference(frame, previous_frame)
                    significant_change = frame_diff > frame_diff_threshold
                    
                    # Only run inference if there's significant change or no previous frame
                    if significant_change or previous_frame is None:
                        def run_inference():
                            return model.track(source=frame, persist=True, conf=0.3, iou=0.5, verbose=False)
                        ...
                        try:
                            ...
                            logger.warning("Model inference timed out after 2 seconds")
                            results = None
                        except Exception as e:
                        ...
                        # Skip inference due to minimal frame change
                        logger.debug(f"Skipping inference - frame difference: {frame_diff} < threshold: {frame_diff_threshold}")
                        results = None
                ...
                #time.sleep(0.033)
                # Control frame rate and yield to prevent worker timeout
                queue_size = frame_queue.qsize()
                if queue_size > 150:      # Queue filling up
                    time.sleep(0.005)     # Speed up consumption
                elif queue_size < 50:     # Queue getting low  
                    time.sleep(0.02)      # Slow down to let capture catch up
                else:
        ...
        # Clean up any OpenCV windows (though typically not used in streaming)
        pass
