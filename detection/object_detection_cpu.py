...
class ObjDetection:
    ...
    Handles detection of various objects
    """
    
    def __init__(self, model_path="models/yolo11n.pt"):
        """Initialize Object detection with YOLO model"""
        try:
            ...
            try: 
                if whitelist:
                    self.obj_detected_items = tuple(x.strip() for x in whitelist.split(','))
                else:
                    self.obj_detected_items = ('Helmet', 'Safety-Vest')
    ...
    def process_object_detection(self, frame, polygon=None):
        """Process frame for Object detection and return results"""
        if frame is None:
            return None
...
def test_video_source_connectivity(hls_url, timeout=15):
    ...
    try:
        ...
        if hls_url == '0':
            cap = cv2.VideoCapture(int(hls_url))
        else:
            cap = cv2.VideoCapture(hls_url, cv2.CAP_FFMPEG)
...
def process_ppe_video_stream(frame_queue, camera):
    ...
    try:
        while True:
            ...
            detection_result = ppe_detector.process_obj_detection(frame, scaled_polygons[0] if scaled_polygons else None)
