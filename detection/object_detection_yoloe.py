...
def get_model():
    ...
    if det_model is None:
        ...
        #det_model = YOLOE("models/yoloe-11s-seg.pt", task="predict")
        det_model = YOLO("models/yolo11n.mlpackage", task="detect")
        # Load COCO class names from coco8.yaml
        #names = load_class_names_from_yaml()
        #det_model.set_classes(names, det_model.get_text_pe(names))
...
try:
    whitelist = SystemConfig.get_value('obj_det_cls_id_all_whitelist')
    if whitelist:
        list_class = [int(x.strip()) for x in whitelist.split(',')]
    else:
        list_class = [0, 2]  # person=0, car=2 (class IDs as integers)
except Exception as e:
...
def send_notification_to_saras_async(*args):
    Thread(target=send_alert_to_saras, args=args).start()
...
def process_video_stream(frame_queue, camera): 
    ...
    try:
        while True:
                ...
                target_width = 640
                target_height = int(original_height / (original_width / target_width))  # Standard 4:3 ratio that should be supported
                ...
                if len(polygons) > 0:
                    ...
                    for i, polygon in enumerate(polygons):
                        if polygon is not None and len(polygon) >= 3:
                            ...
                            scaled_polygon[:, 0, 0] = np.clip((scaled_polygon[:, 0, 0] * scale_x).astype(np.int32), 0, target_width - 1)
                            scaled_polygon[:, 0, 1] = np.clip((scaled_polygon[:, 0, 1] * scale_y).astype(np.int32), 0, target_height - 1)
                            scaled_polygons.append(scaled_polygon)
                    
                    # Only draw if we have valid scaled polygons
                    if scaled_polygons:
                ...
                if results and len(results) > 0 and results[0].boxes is not None:
                    ...
                    for track in results[0].boxes:
                        ...
                        # Convert cls_id to int for comparison with list_class
                        ...
                        if point_inside_any_polygon:
                            ...
                            if current_time - previous_snapshot_time >= snapshot_timer:
                                ...
                                # send notification to SARAS with temporary data
                                
                                previous_snapshot_time = time.time()
                                logger.info(f"Security intrusion detected: {enhanced_message}")                           

                        # Map class ID to class name (loaded from coco8.yaml)
                ...
                # Control frame rate for natural playback
                time.sleep(get_time_sleep())  # ~50 FPS (15ms delay)
