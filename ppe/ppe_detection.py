...
from detection.save_snapshots import save_snapshot_and_record, save_detection_ppecheck, generate_frameid
...
def get_time_sleep():
    ...
    if _time_sleep is None:
        try:
            _time_sleep = float(SystemConfig.get_value('frame_proc_delay', 0.033))
        except:
            _time_sleep = 0.033  # Default fallback
    return _time_sleep
...
class PPEDetection:
    ...
    def __init__(self, model_path="models/yolov8s_custom.mlpackage"):
    ...
    def check_ppe_compliance(self, person_bbox, ppe_detections):
        ...
        ppe_items = []
        overlap_threshold = 0.3  # Minimum 20% overlap required
    ...
    def draw_roi_polylines(self, frame, polygons, transparency=0.1):
        """
        ...
        for i, polygon in enumerate(polygons):
            if polygon is not None and len(polygon) > 2:
                # Fill polygon with white color on overlay
                fill_color = (255, 255, 255)  # White color (BGR format)
    ...
    def store_ppe_detections_in_roi(self, cameraid, detection_id, detection_results, frame):
        """Store all PPE detection objects that are entering ROI"""
        from django.utils import timezone
        
        for detection in detection_results:
            if detection.get('in_roi', False):
                # Generate frameid for this PPE detection
                current_timestamp = timezone.now()
                frameid = generate_frameid(cameraid, current_timestamp)
                
                # Convert missing items to lowercase once for case-insensitive comparison
                missing_lower = [item.lower() for item in detection.get('missing_items', [])]

                # Create PPE data structure for saving
                ppe_data = {
                    'trackid': detection['track_id'],
                    'has_helmet': 'helmet' not in missing_lower,
                    'has_vest': not any(x in missing_lower for x in ['safety-vest', 'vest', 'protective_suit']),
                    'has_goggles': not any(x in missing_lower for x in ['goggles', 'glass', 'googles']),
                    'has_gloves': 'gloves' not in missing_lower,
                    'has_shoes': not any(x in missing_lower for x in ['safety-shoes', 'safety-boot']),
                    'details': {
                        'bbox': detection['bbox'],
                        'missing_items': detection.get('missing_items', []),
                        'violation_type': detection.get('violation_type', 'ppe_missing'),
                        'timestamp': current_timestamp.isoformat(),
                        'frameid': frameid
                    }
                }
                
                # Save PPE check data using the function from save_snapshots.py
                try:
                    save_detection_ppecheck(frame, cameraid, detection_id, ppe_data)
                    logger.info(f"PPE detection stored for track_id: {detection['track_id']} with frameid: {frameid} in camera: {cameraid}")
                except Exception as e:
                    logger.error(f"Failed to store PPE detection for track_id {detection['track_id']}: {e}")
    ...
    def process_ppe_detection(self, frame, polygon=None):
        ...
        # Use ORIGINAL frame for YOLO detection (no modifications)
        original_frame = frame.copy()
        
        # Create display frame (resized copy) only for visualization
        display_frame = cv2.resize(frame, (target_width, target_height), interpolation=cv2.INTER_AREA)
        
        # Calculate scaling factors for polygon coordinates and bounding boxes
        scale_x = target_width / original_width
        scale_y = target_height / original_height
        
        # Keep original polygon coordinates for ROI checking, create scaled version for display
        original_polygons = []
        display_polygons = []
        if polygon is not None:
            ...
            display_polygons = [scaled_polygon]

        # Run YOLO detection on ORIGINAL unmodified frame
        results = self.det_model.track(source=original_frame, persist=True, conf=0.5, iou=0.6, verbose=False)
        
        if not results or not results[0].boxes:
            # Draw ROI on display frame even when no detections
            if len(display_polygons) > 0:
                self.draw_roi_polylines(display_frame, display_polygons, 0.3)
            return {'frame': display_frame, 'violations': 0, 'detections': []}
        ...
        for track in results[0].boxes:
            # Get bounding box from original frame
            original_bbox = list(map(int, track.xyxy[0]))
            ...
            # Scale bounding box coordinates to display frame
            x1_orig, y1_orig, x2_orig, y2_orig = original_bbox
            x1_display = int(x1_orig * scale_x)
            y1_display = int(y1_orig * scale_y)
            x2_display = int(x2_orig * scale_x)
            y2_display = int(y2_orig * scale_y)
            display_bbox = [x1_display, y1_display, x2_display, y2_display]

            if cls_name.lower() == "person":
                person_detections.append({
                    'bbox': original_bbox,  # Store original bbox for ROI checking
                    'display_bbox': display_bbox,  # Store scaled bbox for drawing
                    'track_id': track_id,
                    'confidence': confidence
                })
            elif cls_name in self.required_safety_items:
                ppe_detections.append((original_bbox, cls_name))
                # Draw PPE detection with blue box on display frame using scaled coordinates
                self.draw_blue_box(display_frame, track_id, cls_name, confidence, 
        ...
        for person in person_detections:
            original_bbox = person['bbox']  # Original coordinates for ROI checking
            display_bbox = person['display_bbox']  # Scaled coordinates for drawing
            track_id = person['track_id']
            confidence = person['confidence']
            
            ppe_status = self.check_ppe_compliance(original_bbox, ppe_detections)
            
            # Use original coordinates for ROI checking
            x1_orig, y1_orig, x2_orig, y2_orig = original_bbox
            center_point = ((x1_orig + x2_orig) // 2, (y1_orig + y2_orig) // 2)
            
            # Use display coordinates for drawing
            x1_disp, y1_disp, x2_disp, y2_disp = display_bbox
            
            if not ppe_status['is_compliant']:
                violations += 1
                missing_items = ppe_status['missing_items']
                
                # Draw violation box on display frame using scaled coordinates
                label_name = f"Missing [{','.join(missing_items)}]"
                self.draw_blue_box(display_frame, track_id, label_name, confidence, 
                                 x1_disp, y1_disp, x2_disp, y2_disp, color_type='violation', thickness=1)
                
                # Check if in ROI using original coordinates and original polygon
                if len(original_polygons) == 0 or any(self.is_point_in_polygon(center_point, poly) for poly in original_polygons if poly is not None):
                    self.tracked_ids_with_missing_ppe.add(track_id)
                    
                    detection_results.append({
                        'track_id': track_id,
                        'bbox': original_bbox,  # Use original bbox for data storage
                        'missing_items': missing_items,
                        'in_roi': True,
                ...
                if len(detected_items) > 0:
                    ...
                    self.draw_blue_box(display_frame, track_id, label_name, confidence, 
                                     x1_disp, y1_disp, x2_disp, y2_disp, color_type='safe', thickness=1)
                else:
        ...
        # Draw ROI overlay on display frame AFTER all detections are processed
        if len(display_polygons) > 0:
            logger.debug(f"Drawing ROI with {len(display_polygons)} polygons")
            self.draw_roi_polylines(display_frame, display_polygons, 0.3)
        else:
            logger.debug("No ROI polygons to draw")
...
def save_ppe_detections_async(cameraid, detection_id, violations_in_roi, frame):
    """Async wrapper for storing PPE detections"""
    def store_ppe_data():
        try:
            ppe_detector = get_ppe_detector()
            ppe_detector.store_ppe_detections_in_roi(cameraid, detection_id, violations_in_roi, frame)
        except Exception as e:
            logger.error(f"Failed to store PPE detections asynchronously: {e}")
    
    Thread(target=store_ppe_data).start()

def save_snapshot_with_ppe_async(frame, cameraid, message_body, detection_type, object_count, violations_in_roi):
    """
    Enhanced async wrapper that saves snapshot and then stores PPE detections
    Returns detection_id through a callback mechanism
    """
    def save_and_store():
        try:
            # Save snapshot and get detection ID
            detection_id = save_snapshot_and_record(frame, cameraid, message_body, detection_type, object_count)
            
            if detection_id and violations_in_roi:
                # Store PPE detections using the detection ID
                ppe_detector = get_ppe_detector()
                ppe_detector.store_ppe_detections_in_roi(cameraid, detection_id, violations_in_roi, frame)
                logger.info(f"PPE detections stored for detection ID: {detection_id}")
            
        except Exception as e:
            logger.error(f"Failed to save snapshot and store PPE detections: {e}")
    
    Thread(target=save_and_store).start()
...
def capture_video(hls_url, frame_queue):
    ...
    while True:
        ...
        try:
            ...
            cap.set(cv2.CAP_PROP_FPS, 20)  # Set consistent FPS
            ...
            while True:
                ...
                if frame_queue.qsize() > 50:
                    logger.debug("Frame queue approaching capacity, clearing old frames")
                    # Clear some frames to prevent memory buildup
                    try:
                        while frame_queue.qsize() > 40:
                            frame_queue.get_nowait()
                    except:
                        pass

                # Add frame to processing queue with non-blocking approach
                try:
                    frame_queue.put_nowait(frame)
                except:
                    # Skip frame instead of warning to reduce log spam
                    continue
...
def create_robust_capture_with_monitoring(hls_url, frame_queue, status_callback=None, shared_status=None):
    ...
    # Pre-test RTSP connectivity with enhanced retry logic
    test_result = test_video_source_connectivity(hls_url, timeout=20, max_retries=3)
    if not test_result['connected']:
        retry_info = f"after {test_result.get('retry_count', 0)} attempts"
        error_msg = f"RTSP connectivity pre-test failed {retry_info}: {test_result['error']}"
        logger.error(f"Initial RTSP connectivity test failed: {error_msg}")
        ...
        try:
            ...
            while True:
                ...
                if not ret or frame is None:
                    ...
                    time.sleep(0.1)  # Use configured delay
                    continue
                ...
                # Queue management - more aggressive cleanup
                if frame_queue.qsize() > 50:
                    logger.debug("Clearing frame queue backlog")
                    try:
                        while frame_queue.qsize() > 40:
                            frame_queue.get_nowait()
                    except:
                ...
                # Add frame to queue with non-blocking approach
                try:
                    frame_queue.put_nowait(frame)
                except:
                    # Skip frame silently to avoid log spam
                    continue
...
def test_video_source_connectivity(rtsp_url, timeout=15, max_retries=3):
    """
    Test RTSP/HLS video source connectivity with retry logic
    ...
    retry_delays = [2, 5, 10]  # Progressive delays for RTSP connections
    ...
    for attempt in range(max_retries):
        ...
        try:
            logger.info(f"Testing connectivity to RTSP source (attempt {attempt + 1}/{max_retries}): {rtsp_url}")
            
            # Initialize video capture with progressive timeout increases
            current_timeout = timeout + (attempt * 5)  # Increase timeout with each attempt
            
            if rtsp_url[0:5].lower() == 'local':
                cap = cv2.VideoCapture(int(rtsp_url[6]))
            else:
                cap = cv2.VideoCapture(rtsp_url, cv2.CAP_FFMPEG)
                cap.set(cv2.CAP_PROP_OPEN_TIMEOUT_MSEC, current_timeout * 1000)
                cap.set(cv2.CAP_PROP_READ_TIMEOUT_MSEC, 8000 + (attempt * 2000))  # Progressive read timeout
            
            # Wait a bit for RTSP handshake
            if attempt > 0:
                time.sleep(1 + attempt)  # Progressive wait time
            
            if not cap.isOpened():
                error_msg = f"Failed to open video source on attempt {attempt + 1}"
                logger.warning(error_msg)
                result['error'] = error_msg
                
                # Release and wait before retry
                if cap is not None:
                    try:
                        cap.release()
                    except:
            ...
            # Try to read a test frame with multiple attempts
            frame_read_attempts = 3
            frame_success = False
            
            for frame_attempt in range(frame_read_attempts):
                try:
                    ret, frame = cap.read()
                    if ret and frame is not None and not is_blank_frame(frame):
                        result['frame_readable'] = True
                        frame_success = True
                        logger.info(f"RTSP source test successful on attempt {attempt + 1}: {rtsp_url}")
                        break
                    else:
                        if frame_attempt < frame_read_attempts - 1:
                            logger.debug(f"Frame read attempt {frame_attempt + 1} failed, retrying...")
                            time.sleep(1)  # Wait between frame read attempts
                except Exception as frame_error:
                    logger.warning(f"Frame read error on attempt {frame_attempt + 1}: {frame_error}")
                    if frame_attempt < frame_read_attempts - 1:
                        time.sleep(1)
            ...
            if frame_success:
                ...
                result['error'] = "Cannot read frames from RTSP source"
                logger.warning(f"RTSP source connected but frames not readable on attempt {attempt + 1}: {rtsp_url}")
                
        except Exception as e:
            error_msg = f"Video source test failed on attempt {attempt + 1}: {str(e)}"
            result['error'] = error_msg
        ...
        if attempt < max_retries - 1:
            delay = retry_delays[attempt] if attempt < len(retry_delays) else retry_delays[-1]
            logger.info(f"RTSP connection attempt {attempt + 1} failed, waiting {delay} seconds before retry...")
            time.sleep(delay)
    
    # All attempts failed
    result['connected'] = False
    if not result['error']:
        result['error'] = f"Failed to connect to RTSP source after {max_retries} attempts"
...
def start_detection_stream(camera, hls_url):
    ...
    # Test connectivity first with enhanced retry logic
    logger.info(f"Testing RTSP connectivity for camera {camera.name}...")
    connectivity_test = test_video_source_connectivity(hls_url, timeout=15, max_retries=5)
    
    if not connectivity_test['connected']:
        retry_info = f"after {connectivity_test.get('retry_count', 0)} attempts"
        error_msg = f"RTSP connectivity test failed {retry_info}: {connectivity_test['error']}"
        logger.warning(f"Initial connectivity test failed for camera {camera.name}: {error_msg}")
        yield create_status_frame(f"Connection failed {retry_info}")
        return
    
    logger.info(f"RTSP connectivity test successful for camera {camera.name} on attempt {connectivity_test.get('retry_count', 1)}")
    ...
    # Wait for capture to start with extended timeout for RTSP
    logger.info(f"Waiting for RTSP stream to initialize for camera {camera.name}...")
    time.sleep(2.0)  # Initial wait
    
    # Check if capture started successfully with extended patience for RTSP
    max_startup_wait = 15  # Total wait time in seconds
    check_interval = 2     # Check every 2 seconds
    waited_time = 2        # Already waited 2 seconds
    
    while waited_time < max_startup_wait:
        if not frame_queue.empty() or connection_status.get('connected', False):
            logger.info(f"RTSP stream started successfully for camera {camera.name} after {waited_time} seconds")
            break
            
        logger.debug(f"Waiting for RTSP stream... ({waited_time}/{max_startup_wait}s)")
        time.sleep(check_interval)
        waited_time += check_interval
    
    # Final check after waiting
    if frame_queue.empty() and not connection_status.get('connected', False):
        error_msg = connection_status.get('error', 'Failed to start video capture after extended wait')
        logger.error(f"PPE video capture failed to start for camera {camera.name} after {max_startup_wait}s: {error_msg}")
        connection_status['stop_requested'] = True
        yield create_status_frame(f"Stream startup timeout after {max_startup_wait}s")
        return
...
def process_ppe_video_stream(frame_queue, camera):
    ...
    try:
        while True:
            ...
            # Scale polygons to match resized frame for detection
            scaled_polygons = []
            if len(polygons) > 0:
            ...
            # Process PPE detection - ROI drawing now handled inside the detection function
                ...
                if detection_result.get('violations', 0) > 0 and len(detection_result.get('detections', [])) > 0:
                    ...
                    if violations_in_roi:
                        ...
                        if current_time - previous_snapshot_time >= snapshot_timer:
                            ...
                            # Save snapshot and store PPE detections asynchronously
                            save_snapshot_with_ppe_async(processed_frame, cameraid, enhanced_message, "PPE", violation_count, violations_in_roi)
